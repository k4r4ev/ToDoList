...
class Body extends React.Component {
  ...
  setTaskOrder = (desks = this.props.desks) => {
    ...
    for (let deskItem = 0; deskItem < desks.length; deskItem++) {
      for (
        let taskItem = 0;
        taskItem < desks[deskItem].tasks.length;
        taskItem++
      ) {
  ...
  deleteDesks = () => {
    if (this.props.desksIdToDelete.length !== 0) {
      this.props.desksIdToDelete.map((current) =>
        this.props.deleteDesk(current)
      );
      this.desksId = [];
  ...
  deleteAllDesks = () => {
    this.setState({
      modalWindow: (
        <div>
          <Modal desksId={this.desksId} hideModal={this.hideModal} />
          <Overlay />
        </div>
      ),
    });
      ...
      <div>
        ...
        <div className="panel">
          ...
          <div className="header">
            <TextField
              ...
              color="secondary"
              size="small"
              onChange={this.handleChangeDeskNameText}
              value={this.state.deskText}
            />
        ...
        <div className="container">
          {this.props.desks.map((currentDesk, index) =>
            this.prepareDesk(currentDesk, index)
          )}
        </div>
      </div>
    );
